...
const CountChartContainer = () => {
  ...
  useEffect(() => {
    async function fetchResidents() {
      const { data, error } = await supabase
        .from("residents")
        .select("carelevel"); // Adjust the column name if it differs in your DB
  ...
  // Aggregate care levels, ignoring case
  const low = residents.filter(
    (r) => r.carelevel?.toUpperCase() === "LOW"
  ).length;
  const medium = residents.filter(
    (r) => r.carelevel?.toUpperCase() === "MEDIUM"
  ).length;
  const high = residents.filter(
    (r) => r.carelevel?.toUpperCase() === "HIGH"
  ).length;
